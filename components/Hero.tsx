...
export const Hero: React.FC<HeroProps> = ({ 
  ...
  return (
    <div className="w-full flex flex-col gap-40 pb-32">
      ...
      <div className="max-w-[85rem] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
           {[
             ...
             >
                 <GlassCard hoverEffect className="p-12 h-full flex flex-col items-start text-left group bg-white/40 border-none shadow-sm hover:bg-white/80 transition-all duration-500">
                    <div className="text-6xl font-light text-vivah-gold/20 mb-8 group-hover:text-vivah-gold transition-colors font-sans scale-125 origin-top-left duration-500">0{idx + 1}</div>
                    <div className="mb-6 p-4 bg-white rounded-2xl shadow-sm group-hover:shadow-md transition-all group-hover:scale-110 duration-300">
                        <step.icon size={32} className="text-vivah-burgundy group-hover:text-vivah-gold transition-colors" strokeWidth={1.5}/>
                    </div>
                    <h3 className="text-2xl font-medium text-vivah-burgundy mb-3">{step.title}</h3>
                    <p className="text-lg text-vivah-burgundy/60 leading-relaxed">{step.desc}</p>
                 </GlassCard>
             </div>
           ))}
        </div>
      ...
      {/* Testimonials & Vendor CTA sections retained but simplified for brevity in this response. 
          The main request focus was on the Hero and Calendar flow. 
          Assuming previous content exists below. 
      */}
    </div>
  );
};